...
A mutating Codex or Claude session SHALL attempt one shell-free, memory- and lifetime-bounded `--version` observation before SkillMount state access. The observer SHALL prepare dedicated native containment before spawn, attach the root immediately after spawn under ADR 0019's platform boundary, stop each reader at the first byte beyond its retention bound, force the process domain before reaping its root when capture completes or a bound is crossed, and require bounded reader shutdown plus signal-free domain-death proof. A containment, lifetime, spawn, status, size, or encoding failure is unavailable evidence. An exact last-tested banner is silent; a different, malformed, nonzero, oversized, non-UTF-8, or unavailable result emits one bounded warning that identifies the last-tested evidence and MUST NOT by itself prevent staging or launch or change the eventual child/cleanup exit status. The observation is ephemeral and MUST NOT enter a plan, lock key, journal, ownership record, cleanup decision, or compatibility claim.
...
- Windows retains ADR 0019's immediate post-spawn Job assignment window. Reader and wall-clock bounds prevent the wrapper from waiting indefinitely, but a process that deliberately escapes before attachment remains outside the Job's domain-death proof.
...
- Shared version-observation unit tests cover exact and different banners, nonzero status, spawn failure, reader closure at oversized output, invalid UTF-8, and bounded rendering.
- `tests/codex_session.rs` uses the real fake-Agent process to cover oversized interleaved streams and a descendant that inherits both captured handles; it proves bounded termination, process-domain death before launch, one warning, and normal child/cleanup behavior. The Codex and Claude session suites also prove one observation, child-status precedence, inherited session streams, `--keep-mounts`, and normal ownership-verified cleanup.
