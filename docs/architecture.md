...
as current only after it exists in the tracked source and has corresponding verification.
...
inspects every discovery scope the selected agent searches, plans one link per selected Skill, and
owns only the entries its transaction created.
...
A normal mutating invocation therefore does not yet run Codex or Claude. It returns internal exit
category 70 after releasing its mounts, rather than leaving mounts for a child that never started.
...
| `src/state.rs` | Computes and, only after the mutation boundary, creates private state locations. |
...
values. It never invokes a platform backend.

The shared application layer owns ordering and policy. The transaction layer owns multi-step
mutation and durable ownership. The sealed link backend provides narrow platform primitives and
does not decide when a plan should be applied, recovered, or cleaned up. There is no recursive
removal operation in the link contract.

`src/link/` therefore sits below the plan and transaction layers rather than inside the read-only
pipeline:

```text
catalog + agent snapshot -> mount plan -> transaction policy
                                           |
                                           v
                                  sealed link primitives
```
...
of a project-owned or otherwise pre-existing Skill. Planning inspects the complete namespace the
child would search, not just the destination directory, because relying on undocumented duplicate
precedence would make the selected Skill ambiguous.
...
|---|---|---|
| Authoritative discovery | `.agents/skills` from launch CWD through project root | Project and user `.claude/skills`, plus user-supplied `--add-dir` scopes |
...
| Planned destination | Project discovery/backing store chosen by the Codex state table | Unique state-root staging tree at `<session>/root/.claude/skills` |
| Project mutation | Transaction-owned entries may be added to the selected project store | Default staging mode does not modify project `.claude/skills` |
| Launch integration | Reserved | Reserved; the future command will inject `<session>/root` with `--add-dir` |
...
A transaction persists its journal before each filesystem mutation can become externally visible.
The journal distinguishes intent, staged identity, final placement, active use, cleanup, kept state,
and failure. Its path codec round-trips arbitrary Unix bytes and Windows UTF-16, including unpaired
...
Rollback and ordinary cleanup share the same reverse-order removal path. Recovery eligibility comes
from the held lock set, never a recorded PID: PIDs are reusable and cannot authorize cleanup.
Unreadable or future-schema journals block new mutation and are retained for operator inspection.
...
2. Source occurrences are rightmost-wins, validate-after-select, with no fallback from an invalid
   winner.
3. Source precedence never overrides a pre-existing Skill visible to the agent.
...
6. No filesystem mutation occurs before durable intent, and apply rechecks persisted preconditions.
7. No entry is removed without matching kind, target, and available platform identity; uncertainty
   is retained and reported rather than guessed away.
8. Link removal never recursively descends into a target directory.
9. Shared code plans and applies; agent and platform adapters inspect and describe their specific
   behavior.
...
- complete Codex and Claude session adapters built on that supervisor;
...
exit category 70. Until operator cleanup is implemented, lock and owner-sidecar files accumulate one
pair per distinct resource; their presence does not block or prove a live session.
...
- ignored local design documents are proposal or historical inputs, never required checkout
  context.
