...
Every `LockResource` SHALL carry explicit `LockAccess::Observe` or `LockAccess::Mutate` intent. `Observe` covers namespace evidence SkillMount only reads; `Mutate` covers every logical and physical identity that a transaction may create, place into, exclude competing mutation from, prune, or remove. Adapters describe every discovery, destination, and helper identity known from their snapshot. After the shared sequencer has minted the transaction identifier and built the exact plan, shared transaction code SHALL derive an additional volume-root `DiscoveryEntry::Mutate` resource for every transaction-unique staged sibling and stabilize the expanded set before journaling or apply. Adapters describe intent but do not predict transaction-specific names, while shared application and transaction code remains the sole authority for acquisition, recovery, journaling, apply, supervision, and cleanup.

Access SHALL NOT participate in the existing `skillmount-lock-v1` logical or physical key derivation. Every snapshot-known discovery entry whose `origin/dev/0.3.x` producer emitted a project-anchor or deepest-existing identity SHALL retain that exact identity beside the volume-root identity, with the same access. Newly classified observation entries likewise carry the matching legacy anchor convention so they intersect older Codex or OMP producers where those producers used it. Acquiring both logical keys preserves existing live mixed-version intersections while the volume-root key joins new cross-project readers and writers. Exact transaction-specific staged siblings use only their canonical volume-root identity because the older release never emitted them. Requests that collapse to one key retain all diagnostic paths and fold to the strongest access, with `Mutate` stronger than `Observe`. Observation uses a shared operating-system lock; mutation uses an exclusive lock. Both remain held through the child lifetime and exactly one cleanup callback.
...
New journals SHALL use schema version 2 and record access on every lock. Schema version 2 SHALL require every creating action's canonical transaction-unique staged sibling to have mutation authority under the exact volume-root logical key that an external observer derives; an ancestor resource or alternate anchor/suffix split is insufficient. Schema version 1 remains readable by mapping every legacy record to `Mutate` and retaining its conservative legacy authority rules because it did not record staged-sibling keys. Transaction opening and recovery SHALL require held mutation access for every owned mutation identity; compatible observation access is never cleanup authority.
...
- Adapter reviews must classify every snapshot-known observed and mutation-capable identity, including helper chains and cross-Agent logical spellings, retain each replaced `origin/dev/0.3.x` identity beside the shared volume-root identity, and give newly classified observations the corresponding legacy anchor convention. Shared stabilization must add the exact transaction-id-specific staged-sibling identities after plan construction and before journaling. Transaction assertions fail closed if either classification or derivation misses mutation authority.
...
The decision is enforced by access/key aggregation and lock-acquisition tests in `src/lock/`, schema and access-drift tests in `src/journal/` and `src/transaction/`, adapter resource fixtures, and real-process Codex, Claude, OMP, recovery, read-only, and supervision suites. Focused deterministic coverage checks representative Codex, OMP, and Claude observation and mutation resources for both their shared and retained or compatibility logical keys, and checks that a new observer contends with an `origin/dev/0.3.x` writer key. Separate coverage checks that a transaction records the same exact staged-path key an external observer derives, that such an observer blocks recovery mutation, and that schema version 2 rejects a staged-path record using an alternate project-root anchor even when the joined pathname is identical. Acceptance requires native shared/shared overlap, shared/exclusive contention, crash release, doctor observation, sibling-project child overlap, same-destination and physical-alias serialization, and independent ownership-verified cleanup.
