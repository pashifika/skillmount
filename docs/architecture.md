...
| `asm cleanup --project-root <path>` | Reconciles every structurally valid, non-completed journal for the canonical project after taking that journal's complete recorded lock set. A `supervising` or kept journal is eligible only because invoking this command is the operator's assertion that its process domain is dead or its retained mounts may be released. Its stdout report names every removed link, every unresolved cleanup-critical entry, and every helper directory it preserved. |
| `asm cleanup --all` | Applies the same journal-by-journal engine across the bounded state store. Corrupt state blocks all mutation; live locks retain their journals and produce category 75; an unresolved cleanup-critical entry or filesystem failure takes category 73 precedence. Preserved scaffolding alone is reported and returns success. |
...
A genuine session-cleanup failure, a quarantined-journal refusal, and an explicit-cleanup retry are
each rendered once as a structured block: a severity or summary line, then indented fields naming the
reason, every retained path, the retained journal, the process-death precondition, and a labelled
recovery vector whose executable and numbered arguments occupy one line each. Only those fixed labels
and separators insert a line break; every external value is escaped independently through
`src/render.rs`, so no path or reason can forge a line, and nothing is ever joined into a shell
command string.
...
| `src/render.rs` | Read-only plans, normal/verbose session diagnostics, warnings, labelled native argument vectors, and reversible native-value rendering. |
